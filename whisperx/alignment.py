    ...
    for sdx, segment in enumerate(transcript):
        ...
        for cdx, char in enumerate(text):
            ...
            if cdx in segment["clean_cdx"]:
                ...
                score = round(char_seg.score, 3) if include_scores else None

            res = {"char": char, "start": start, "end": end, "word-idx": word_idx}
            if include_scores:
                res["score"] = score
            char_segments_arr.append(res)
        ...
        for sdx, (sstart, send) in enumerate(segment["sentence_spans"]):
            ...
            for word_idx in curr_chars["word-idx"].unique():
                ...
                word_score = (
                    round(word_chars["score"].mean(), 3) if include_scores else None
                )
                ...
                if include_scores and not np.isnan(word_score):
